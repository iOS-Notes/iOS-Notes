...
+ (instancetype)my_ModelWithJSON:(id)json {
    ...
    return [self my_ModelWithDictonary:dic];
}
...
+ (instancetype)my_ModelWithDictonary:(NSDictionary *)dic {
    ...
    // dictonary to model
    [model my_setModelWithDictonary:dic];
...
- (void)my_setModelWithDictonary:(NSDictionary *)dic {
    ...
    // 获取 当前类信息
    MYClassInfo *classInfo = [[MYClassInfo alloc] initWithClass:object_getClass(self)];
    ...
    // 遍历当前类所有属性
    [classInfo.propertyInfo enumerateKeysAndObjectsUsingBlock:^(NSString *key, MYPropertyInfo *propertyInfo, BOOL * stop) {
        ...
        if (!isIgnoreProperty && value) {
            ...
            if ([value isKindOfClass:[NSArray class]]) {
                ...
                if ([propertyInfo.typeClass isSubclassOfClass:[NSArray class]]) {
                    ...
                    if (class) {
                        // 包含 就调用数组的 转模型方法
                        value = [(NSArray *)value my_ModelArrayWithClass:class];
                ...
                if (propertyInfo.isCustomFondation) {
                    // 字典 对应模型
                    value = [propertyInfo.typeClass my_ModelWithDictonary:value];
                    ...
                    if (class) {
                        // 包含 就调用字典的 转模型方法
                        value = [(NSDictionary *)value my_ModelDictionaryWithClass:class];
                    } else if ([self respondsToSelector:@selector(shouldCustomUnkownValueWithKey:)] && [self shouldCustomUnkownValueWithKey:key] && [self respondsToSelector:@selector(customValueWithKey:unkownValueDictionary:)]) {
                        // 自定义处理未知value
                        value = [self customValueWithKey:key unkownValueDictionary:value];
                    }
                } else {
    ...
    [classInfo.propertyInfo enumerateKeysAndObjectsUsingBlock:^(NSString *key, MYPropertyInfo *propertyInfo, BOOL * stop) {
        ...
        if (!isIgnoreProperty && value) {
            ...
            if ([value isKindOfClass:[NSArray class]]) {
                ...
                value = [(NSArray *)value my_ModelArrayToDicArray];
            } else if ([value isKindOfClass:[NSDictionary class]]) {
                // 如果值是字典（可能包含模型） 调用字典 model转dic方法
                value = [(NSDictionary *)value my_ModelDictionaryToDictionary];
            } else if (propertyInfo.typeClass && propertyInfo.isCustomFondation) {
                value = [value my_ModelToDictonary];
            }
...
+ (NSArray *)my_ModelArrayWithDictionaryArray:(NSArray *)dicArray {
    return [dicArray my_ModelArrayWithClass:[self class]];
}

#pragma mark - model to json
// model to json
- (id)my_ModelToJSONObject {
    if ([self isKindOfClass:[NSArray class]]) {
        // 如果是数组
        return [(NSArray *)self my_ModelArrayToDicArray];
    }
    
    return [self my_ModelToDictonary];
}

- (NSData *)my_ModelToJSONData {
    id jsonObject = [self my_ModelToJSONObject];
    if (!jsonObject) {
        return nil;
    }
...
- (NSString *)my_ModelToJSONString {
    NSData *jsonData = [self my_ModelToJSONData];
...
+ (NSArray *)my_DictionaryArrayWithModelArray:(NSArray *)dicArray {
    return [dicArray my_ModelArrayToDicArray];
}
...
@implementation NSArray (MYJSONModel)

// 数组 to model 数组
- (NSArray *)my_ModelArrayWithClass:(Class)class {
    if (!class) {
        return self;
    }
    ...
    for (id value in self) {
        ...
        if ([value isKindOfClass:[NSDictionary class]]) {
            // value是字典 则 字典转模型
            NSObject *obj = [class my_ModelWithDictonary:value];
            if (obj) {
...
@implementation NSDictionary (MYJSONModel)

// 字典 to model 字典
- (NSDictionary *)my_ModelDictionaryWithClass:(Class)class {
    if (!class) {
        return self;
    }
    ...
    [self enumerateKeysAndObjectsUsingBlock:^(NSString *key, id obj, BOOL * stop) {
        if ([key isKindOfClass:[NSString class]]) {
            // 字典包含 obj字典 转 model
            NSObject *value = [class my_ModelWithDictonary:obj];
            if (value) {
                dic[key] = value;
            }
        }
